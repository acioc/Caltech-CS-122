    ...
    public Object evaluate(Environment env) throws ExpressionException {
        ...
        switch (type) {
            case LIKE:
                ...
                String lhsRegex = Pattern.quote(lhsValue);
                lhsRegex = lhsRegex.replaceAll("_", ".").replaceAll("%", ".*");

                logger.info("Received LIKE pattern \"" + lhsValue +
                    "\", converted to regex \"" + lhsRegex + "\".");

                result = Pattern.matches(lhsRegex, rhsValue);
                break;

            case REGEX:
                try {
                    result = Pattern.matches(lhsValue, rhsValue);
                }
                catch (PatternSyntaxException e) {
                    throw new ExpressionException("Regex pattern is invalid!", e);
                }
                break;
